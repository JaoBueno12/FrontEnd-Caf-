// Array principal com todos os dados do menu organizados por categorias
const MENU = [
  // Seção de bebidas
  {
...
// Cria array com todos os produtos (achata as seções em um array único)
const PRODUCTS = MENU.map((item) => item.data).flat();

// Cria array com os nomes das categorias
const CATEGORIES = MENU.map((item) => item.title);

// Define o tipo TypeScript baseado no primeiro produto do array
type ProductProps = (typeof PRODUCTS)[0];

// Exporta todas as constantes e tipos para uso em outros arquivos
